...
describe('Gallery content contract', () => {
  it('round-trips semantic gallery content', () => {
    ...
    expect(parseGallery(node)).toEqual({
      images: [{
        id: 'mountain',
        src: '/mountain.jpg',
        alt: 'Mountain',
  ...
  it('migrates legacy selectedImages only when explicitly requested', () => {
    ...
    legacy.setAttribute('data-brickdata', JSON.stringify({
      selectedImages: [{ id: 'legacy-1', url: '/legacy.jpg', title: 'Legacy image' }],
    }));
    legacy.innerHTML = '<img src="/legacy.jpg" title="Legacy image" alt="Legacy alt">';

    expect(parseLegacyGallery(legacy)).toEqual({
      images: [{ id: 'legacy-1', src: '/legacy.jpg', alt: 'Legacy alt', title: 'Legacy image' }],
    });
